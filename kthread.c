...
static inline long steal_map_work(kt_for_map_t *t)
{
	int i, min_i = -1;
	long k, min = LONG_MAX;
	for (i = 0; i < t->n_threads; ++i)
...
static void *ktf_worker_map(void *data)
{
...
void kt_for_map(int n_threads, void (*func)(void*,long,int,void*), void *data, long n, void* params, void (*last_func)(void*, int), void* (*extra_func)(void*))
{
	if (n_threads > 1) {
		...
		for (i = 0; i < n_threads; ++i) pthread_create(&tid[i], 0, ktf_worker_map, &t.w[i]);
		for (i = 0; i < n_threads; ++i) pthread_join(tid[i], 0);
		free(tid); free(t.w);
	} else {
		long j;
		for (j = 0; j < n; ++j) func(data, j, 0, params);
	}
}
